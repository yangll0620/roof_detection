...
class SSDMetaArch(model.DetectionModel):
  ...
  def _raw_detections_and_feature_map_inds(self, box_encodings, boxlist_list):
    ...
    return {
        standard_fields.PredictionFields.raw_detection_boxes: raw_detection_boxes,
        standard_fields.PredictionFields.raw_detection_feature_map_indices:
  ...
  def postprocess(self, prediction_dict, true_image_shapes):
    ...
    with tf.name_scope('Postprocessor'):
      ...
      if detection_keypoints is not None:
        ...
        additional_fields[standard_fields.BoxListFields.keypoints] = detection_keypoints
      ...
      detection_dict = {
          ...
          standard_fields.DetectionResultFields.detection_classes:
              nmsed_classes,
          standard_fields.DetectionResultFields.num_detections:
              tf.cast(num_detections, dtype=tf.float32),
          standard_fields.DetectionResultFields.raw_detection_boxes:
              tf.squeeze(detection_boxes, axis=2),
          standard_fields.DetectionResultFields.raw_detection_scores:
              detection_scores_with_background
      }
      if (nmsed_additional_fields is not None and
          standard_fields.InputDataFields.multiclass_scores in nmsed_additional_fields):
        detection_dict[
            standard_fields.DetectionResultFields.detection_multiclass_scores] = (
                nmsed_additional_fields[
                    standard_fields.InputDataFields.multiclass_scores])
      if (nmsed_additional_fields is not None and
          'anchor_indices' in nmsed_additional_fields):
        detection_dict.update({
            standard_fields.DetectionResultFields.detection_anchor_indices:
                tf.cast(nmsed_additional_fields['anchor_indices'], tf.int32),
        })
      if (nmsed_additional_fields is not None and
          standard_fields.BoxListFields.keypoints in nmsed_additional_fields):
        detection_dict[standard_fields.DetectionResultFields.detection_keypoints] = (
            nmsed_additional_fields[standard_fields.BoxListFields.keypoints])
      if nmsed_masks is not None:
        detection_dict[
            standard_fields.DetectionResultFields.detection_masks] = nmsed_masks
  ...
  def loss(self, prediction_dict, true_image_shapes, scope=None):
    ...
    with tf.name_scope(scope, 'Loss', prediction_dict.values()):
      keypoints = None
      if self.groundtruth_has_field(standard_fields.BoxListFields.keypoints):
        keypoints = self.groundtruth_lists(standard_fields.BoxListFields.keypoints)
      weights = None
      if self.groundtruth_has_field(standard_fields.BoxListFields.weights):
        weights = self.groundtruth_lists(standard_fields.BoxListFields.weights)
      confidences = None
      if self.groundtruth_has_field(standard_fields.BoxListFields.confidences):
        confidences = self.groundtruth_lists(standard_fields.BoxListFields.confidences)
      (batch_cls_targets, batch_cls_weights, batch_reg_targets,
       batch_reg_weights, batch_match) = self._assign_targets(
           self.groundtruth_lists(standard_fields.BoxListFields.boxes),
           self.groundtruth_lists(standard_fields.BoxListFields.classes),
           keypoints, weights, confidences)
      match_list = [matcher.Match(match) for match in tf.unstack(batch_match)]
      if self._add_summaries:
        self._summarize_target_assignment(
            self.groundtruth_lists(standard_fields.BoxListFields.boxes), match_list)
      ...
      losses_mask = None
      if self.groundtruth_has_field(standard_fields.InputDataFields.is_annotated):
        losses_mask = tf.stack(self.groundtruth_lists(
            standard_fields.InputDataFields.is_annotated))
  ...
  def _assign_targets(self,
    ...
    if groundtruth_keypoints_list is not None:
      for boxlist, keypoints in zip(
        ...
        boxlist.add_field(standard_fields.BoxListFields.keypoints, keypoints)
    if train_using_confidences:
  ...
  def _batch_decode(self, box_encodings, anchors):
    ...
    if decoded_boxes.has_field(standard_fields.BoxListFields.keypoints):
      decoded_keypoints = decoded_boxes.get_field(
          standard_fields.BoxListFields.keypoints)
